...
class Process:
    """
    The main process the executes Minor Abstract Syntax Tree
    """
    ...
    def evaluate(self, parsed):
        ...
        if type(parsed) != tuple:
            ...
            if action == 'class_func_call':
                ...
                self.env.update({ name: MClass(name, class_process.env) })
            elif action == 'typeof':
                try:
                    if len(parsed[1]) == 2:
                ...
                rel_path = 'lib/' + parsed[1] + '.mino'
                path = join(base_dir, rel_path)
                if exists(path):
                    fp = open(path)
                    self.import_contents(fp.read())
            elif action == "extend":
                lib = importlib.import_module(parsed[1])
                extend = {}
                for i in dir(lib):
                    extend.update({i:getattr(lib,i)})
                self.env.update(extend)
            elif action == 'fn':
                params = parsed[2]
                body = parsed[3]
...
class Function(object):
    """
    Function object for Minor Functions and annoymous functions (lambdas)
    """
...
class Value(object):
    """
    Class container for values inside the Minor Language
    """
...
class MInstance(object):
    """
    Object for instances of a class in Minor
    """
    def __init__(self, MClass):
        self.MClass = MClass
        self.type = 'instance'
        init_method = self.MClass.env.get('init')
        if init_method is not None:
            init_method()

    def __str__(self):
        return "<{} instance>".format(self.MClass.name)

    def find_method(self, name):
        return self.MClass.env.find(name)
